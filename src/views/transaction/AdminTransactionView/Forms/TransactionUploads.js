...
function BuyerUploads(value) {
  ...
  async function onpurchaseoChange(newfile) {
    ...
    try {
      const u = newfile ? await s3Up(newfile, 'purchase_order_attachment') : null;
  ...
  async function onblChange(newfile) {
    ...
    setBlLoading(true);
    try {
      const u = newfile ? await s3Up(newfile, 'bill_of_lading_attachment') : null;
  ...
  async function oncargoiChange(newfile) {
    ...
    try {
      const u = newfile ? await s3Up(newfile, 'cargo_insurance_attachment') : null;
