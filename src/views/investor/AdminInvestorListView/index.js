...
import AdminInvestorView from 'src/views/investor/AdminInvestorView';
...
const AdminInvestorListView = () => {
  ...
  React.useEffect(() => {
    async function getInvestors() {
      let filter = { sortkey: { contains: "investor-", notContains: "-investor" } };
      const {
        data: {
  ...
  const handleSelectOne = (event, id) => {
    ...
    if (selectedIndex === -1) {
      newSelectedInvestorIds = newSelectedInvestorIds.concat(selectedInvestorIds, id);
    } else if (selectedIndex === 0) {
      newSelectedInvestorIds = newSelectedInvestorIds.concat(selectedInvestorIds.slice(1));
    } else if (selectedIndex === selectedInvestorIds.length - 1) {
      newSelectedInvestorIds = newSelectedInvestorIds.concat(selectedInvestorIds.slice(0, -1));
    } else if (selectedIndex > 0) {
            ...
            <TableHead>
              <TableRow>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedInvestorIds.length === investor.length}
                    color="primary"
                    indeterminate={
            ...
            <TableBody>
              {investor.slice(0, limit).map((investor) => (
                <TableRow
                  ...
                  selected={selectedInvestorIds.indexOf(investor.investorId) !== -1}
                >
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={selectedInvestorIds.indexOf(investor.investorId) !== -1}
                      onChange={(event) => handleSelectOne(event, investor.investorId)}
                      value="true"
                    />
                  </TableCell>
                  <TableCell>
                    ...
                    >
                      ...
                      <Avatar
                        ...
                        onClick={() => getidandident(investor.investorId, investor.userId, investor.identityId)}
                      >
