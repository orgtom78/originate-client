...
function BuyerUploads(value) {
  ...
  const [buyer_shareholder_list_attachment, setBuyer_shareholder_list_attachment] = useState("");
  const [buyer_director_list_attachment, setBuyer_director_list_attachment] = useState("");
  const [buyer_articles_of_association_attachment, setBuyer_articles_of_association_attachment] = useState("");
  ...
  async function getBuyer(input) {
    try {
      ...
      setBuyer_articles_of_association_attachment(buyer_articles_of_association_attachment);
  ...
  async function onregcertChange(newfile) {
    ...
    try {
      const u = newfile ? await s3Up(newfile, 'buyer_registration_cert_attachment') : null;
  ...
  async function onshareholderlChange(newfile) {
    ...
    try {
      const u = newfile ? await s3Up(newfile, 'buyer_shareholder_list_attachment') : null;
  ...
  async function ondirectorlChange(newfile) {
    ...
    try {
      const u = newfile ? await s3Up(newfile, 'buyer_director_list_attachment') : null;
  ...
  useEffect(() => {
    if (buyer_articles_of_association_attachment) {
      async function getaoaimgurl() {
        var uploadext = buyer_articles_of_association_attachment.split(".").pop();
        var imageExtensions = [
          "jpg",
          "jpeg",
          "bmp",
        ...
        const x = imageExtensions.includes(uploadext);
        if (x === true) {
          const y =  await Storage.get(buyer_articles_of_association_attachment, {
            level: 'private',
            identityId: identityId,
          })
          setAoaImg(y);
  ...
  async function onaoaChange(newfile) {
    ...
    setAoaLoading(true);
    try {
      const u = newfile ? await s3Up(newfile, 'buyer_articles_of_association_attachment') : null;
