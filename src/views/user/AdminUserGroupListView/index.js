...
const AdminUserGroupListView = () => {
  ...
  const handleSelectAll = (event) => {
    ...
    if (event.target.checked) {
      newSelectedUsergroupIds = usergroup.map((usergroup) => usergroup.usergroupId);
    } else {
  ...
  const handleSelectOne = (event, id) => {
    ...
    if (selectedIndex === -1) {
      newSelectedUsergroupIds = newSelectedUsergroupIds.concat(selectedUsergroupIds, id);
    } else if (selectedIndex === 0) {
      newSelectedUsergroupIds = newSelectedUsergroupIds.concat(selectedUsergroupIds.slice(1));
    } else if (selectedIndex === selectedUsergroupIds.length - 1) {
      newSelectedUsergroupIds = newSelectedUsergroupIds.concat(selectedUsergroupIds.slice(0, -1));
    } else if (selectedIndex > 0) {
            ...
            <TableHead>
              <TableRow>
                <TableCell padding="checkbox">
                  <Checkbox
                    checked={selectedUsergroupIds.length === usergroup.length}
                    color="primary"
                    indeterminate={
            ...
            <TableBody>
              {usergroup.slice(0, limit).map((usergroup) => (
                <TableRow
                  ...
                  selected={selectedUsergroupIds.indexOf(usergroup.userId) !== -1}
                >
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={selectedUsergroupIds.indexOf(usergroup.userId) !== -1}
                      onChange={(event) => handleSelectOne(event, usergroup.userId)}
                      value="true"
                    />
                  </TableCell>
                  <TableCell>
