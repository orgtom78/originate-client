...
useEffect(() => { 
  ...
  async function getFinancials(input) {
    try {
      let filter = { userId: { eq: input }, sortkey: {contains: "financials-supplier"} };
      const {data:{listsFinancials:{items:[itemsPage1], nextToken } } } = await API.graphql(graphqlOperation(queries.listsFinancials, { filter: filter }));
      const n = await {data:{listsFinancials:{items:[itemsPage1], nextToken } } };
                  ...
                  <Grid container spacing={3}>
                    ...
                    <Grid item md={6} xs={12}>
                      <TextField
                        ...
                        onChange={(e) => setSupplier_trading_name(e.target.value)}
                        required
                    ...
                    <Grid item md={6} xs={12}>
                      <TextField
                        ...
                        onChange={(e) => setSupplier_register_number(e.target.value)}
                        required
