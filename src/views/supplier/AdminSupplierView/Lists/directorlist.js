...
const DirectorListView = (value) => {
  ...
  useEffect(() => {
    async function getDirectors() {
      const id = await sub;
      let filter = { userId: { eq: id }, sortkey: { contains: "director-supplier" } };
      const {
        data: {
  ...
  const handleSelectOne = (event, id) => {
    ...
    if (selectedIndex === -1) {
      newSelectedDirectorIds = newSelectedDirectorIds.concat(selectedDirectorIds, id);
    } else if (selectedIndex === 0) {
      newSelectedDirectorIds = newSelectedDirectorIds.concat(selectedDirectorIds.slice(1));
    } else if (selectedIndex === selectedDirectorIds.length - 1) {
      newSelectedDirectorIds = newSelectedDirectorIds.concat(selectedDirectorIds.slice(0, -1));
    } else if (selectedIndex > 0) {
            ...
            <TableBody>
              {director.slice(0, limit).map((director) => (
                <TableRow
                  ...
                  selected={selectedDirectorIds.indexOf(director.directorId) !== -1}
                >
                  <TableCell padding="checkbox">
                    <Checkbox
                      checked={selectedDirectorIds.indexOf(director.directorId) !== -1}
                      onChange={(event) => handleSelectOne(event, director.directorId)}
                      value="true"
                    />
                  ...
                  <TableCell>
                    ...
                    >
                      <Avatar
                        ...
                        onClick={() => getidandident(director.sortkey, director.userId)}
                      >
