...
export const UserProvider = ({ children }) => {
  ...
  const [investor_date_of_incorporation, setInvestor_date_of_incorporation] = useState("");
  ...
  const [investor_address_postalcode, setInvestor_address_postalcode] = useState("");
  const [investor_country, setInvestor_country] = useState("");
  const [investor_industry, setInvestor_industry] = useState("");
  const [investor_registration_cert_attachment,setInvestor_registration_cert_attachment] = useState("");
  const [investor_articles_of_association_attachment, setInvestor_articles_of_association_attachment] = useState("");
  const [investor_director_list_attachment, setInvestor_director_list_attachment] = useState("");
  const [investor_shareholder_list_attachment, setInvestor_shareholder_list_attachment] = useState("");
  ...
  React.useEffect(() => {
    ...
    async function loadInvestor() {
      ...
      const { data: { listsInvestor: { items: itemsPage1, nextToken }, },
      } = await API.graphql(
    ...
    async function onLoad() {
      ...
      if (investordata === undefined){
        ...
        setInvestor_registration_cert_attachment(investor_registration_cert_attachment);
        setInvestor_articles_of_association_attachment(investor_articles_of_association_attachment);
        setInvestor_director_list_attachment(investor_director_list_attachment);
        setInvestor_shareholder_list_attachment(investor_shareholder_list_attachment);
      }}
    onLoad();
  }, []);
  ...
  return <InvestorContext.Provider value={values}>{children}</InvestorContext.Provider>;
};
