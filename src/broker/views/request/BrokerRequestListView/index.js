...
const BrokerTransactionListView = () => {
  const classes = useStyles();
  const [request, setRequest] = useState([]);
  const [limit, setLimit] = useState(10);
  const [page, setPage] = useState(0);
  const [brokerid, setInvestid] = useState("");
                          ...
                          <TableRow hover key={request.id}>
                            <TableCell>
                              <Box alignItems="center" display="flex">
                                <Link to={`/broker/request/${request.id}`}>
                                  <Avatar
                            ...
                            <TableCell>{`${request.broker_fee_amount}`}</TableCell>
...
export default BrokerTransactionListView;
