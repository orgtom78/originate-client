...
import TotalBrokerFee from "./TotalBrokerFee";
import BrokerFeeAvg from "./BrokerFeeAvg";
...
const BrokerDashboard = () => {
  ...
  useEffect(() => {
    async function getRequests() {
      let filter = { brokerId: { eq: brokerid } };
      const {
        data: {
    ...
    <Page className={classes.root} title="Dashboard">
      <Container maxWidth={false}>
        <Grid container spacing={3}>
          ...
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <BrokerFeeAvg value={transaction} />
          </Grid>
          <Grid item lg={3} sm={6} xl={3} xs={12}>
            <TotalBrokerFee value={transaction} />
          </Grid>
